...
class InternalTorchIRNode:
    ...
    def __init__(
        ...
        if node is not None:
            ...
            self.inputs_dtype = [_input.type().str() for _input in node.inputs()]
            self.inputs_buf = [_input.toIValue() for _input in node.inputs()]
            self.outputs = [output.debugName() for output in node.outputs()]
            self.outputs_dtype = [output.type().str() for output in node.outputs()]
            self.outputs_buf = [output.toIValue() for output in node.outputs()]
